...
class UserImpl {
  ...
  static _fromJSON(auth2, object) {
    ...
    const email2 = (_b = object.email) !== null && _b !== void 0 ? _b : void 0;
    ...
    assertStringOrUndefined(email2, auth2.name);
...
class EmailAuthCredential extends AuthCredential {
  ...
  static _fromEmailAndPassword(email2, password) {
    return new EmailAuthCredential(email2, password, "password");
  }
  static _fromEmailAndCode(email2, oobCode, tenantId = null) {
    return new EmailAuthCredential(email2, oobCode, "emailLink", tenantId);
  }
  toJSON() {
    return {
      email: this._email,
...
class EmailAuthProvider {
  ...
  static credential(email2, password) {
    return EmailAuthCredential._fromEmailAndPassword(email2, password);
  }
  static credentialWithLink(email2, emailLink) {
    const actionCodeUrl = ActionCodeURL.parseLink(emailLink);
    _assert(actionCodeUrl, "argument-error");
    return EmailAuthCredential._fromEmailAndCode(email2, actionCodeUrl.code, actionCodeUrl.tenantId);
  }
}
...
async function createUserWithEmailAndPassword(auth2, email2, password) {
  const authInternal = _castAuth(auth2);
...
function signInWithEmailAndPassword(auth2, email2, password) {
  return signInWithCredential(getModularInstance(auth2), EmailAuthProvider.credential(email2, password));
}
...
async function signInWithCredentials(emailLogin, passwordLogin) {
  let result = false;
  let message = "Algo va mal";
  try {
    await signInWithEmailAndPassword(auth, emailLogin, passwordLogin);
    message = "Has iniciado sesi\xF3n correctamente";
    result = true;
  } catch (error2) {
    if (error2.code === "auth/wrong-password") {
      message = "La contrase\xF1a que ingresaste es incorrecta";
    } else if (error2.code === "auth/user-not-found") {
      message = "El correo que ingresaste es inv\xE1lido";
    }
  }
  return {
    ok: result,
...
const imgLogo = "/assets/logo.1b342fbe.png";
const imgLogoGoogle = "/assets/logo_google.52a5cef1.png";
const imgLogoPost = "/assets/logopost.d0bc9139.png";
function login(navigateTo2) {
  ...
  textWelcome.textContent = "Aqu\xED encontrar\xE1s opiniones y recomendaciones de las mejores pel\xEDculas";
  ...
  labelEmail.textContent = "Correo electr\xF3nico";
  const email2 = document.createElement("input");
  email2.placeholder = "ejemplo@ejemplo.com";
  email2.type = "email";
  email2.id = "signin_email";
  ...
  const register2 = document.createElement("p");
  register2.className = "question";
  register2.textContent = "\xBFNo tienes una cuenta?   ";
  const registerLink = document.createElement("a");
  registerLink.className = "link";
  registerLink.innerHTML = "Reg\xEDstrate aqu\xED";
  registerLink.href = "";
  register2.appendChild(registerLink);
  ...
  txtGoogle.innerHTML = "Contin\xFAa con Google";
...
function register(navigateTo2) {
  ...
  name2.placeholder = "Ana Martinez";
  name2.type = "text";
  name2.id = "register_name";
  const labelEmail = document.createElement("label");
  labelEmail.textContent = "Correo electr\xF3nico";
  const email2 = document.createElement("input");
  email2.placeholder = "ejemplo@ejemplo.com";
  email2.type = "email";
  email2.id = "register_email";
  email2.value = "";
  ...
  labelConfirm.textContent = "Confirmar contrase\xF1a";
  ...
  returnLogin.textContent = "\xBFYa tienes una cuenta?   ";
  const registerLink = document.createElement("a");
  registerLink.className = "link";
  registerLink.innerHTML = "Inicia Sesi\xF3n";
  registerLink.href = "";
  ...
  signupForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (error2.code === "auth/email-already-in-use") {
        spanMessage.textContent = "El correo ya est\xE1 registrado";
      } else if (error2.code === "auth/invalid-email") {
        spanMessage.textContent = "El correo que ingresaste es inv\xE1lido";
      } else if (error2.code === "auth/weak-password") {
        spanMessage.textContent = "La contrase\xF1a que ingresaste es d\xE9bil";
      } else if (error2.code) {
...
k$1.abort = function() {
  ...
  1 <= this.readyState && this.g && 4 != this.readyState && (this.g = false, id$1(this));
  this.readyState = gd;
};
k$1.Wa = function(a) {
...
k$1.Ta = function(a) {
  if (this.g) {
    ...
    a.done ? id$1(this) : hd(this);
    3 == this.readyState && jd(this);
  }
};
k$1.Va = function(a) {
  this.g && (this.response = this.responseText = a, id$1(this));
};
k$1.Ua = function(a) {
  this.g && (this.response = a, id$1(this));
};
k$1.ga = function() {
  this.g && id$1(this);
};
...
const updatePost = (id2, comment) => rf(rh(db, "posts", id2), comment);
const like = (id2, email2) => {
  const postRef = rh(db, "posts", `${id2}`);
  rf(postRef, {
    likes: Sf(email2)
  });
};
const dislike = (id2, email2) => {
  const postRef = rh(db, "posts", `${id2}`);
  rf(postRef, {
    likes: Df(email2)
  });
};
const postCollection = sh(db, "posts");
const addPost = (comment, likes) => {
  uf(postCollection, {
    comment,
    likes
  });
...
tf(postCollection);
const paintRealTime = (callback) => cf(postCollection, callback);
...
function wall() {
  ...
  signOutBtn.textContent = "Cerrar Sesi\xF3n";
  ...
  txtPost.placeholder = "Escribe aqu\xED el comentario sobre la pel\xEDcula";
  txtPost.id = "textPost";
